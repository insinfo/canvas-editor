...
| F4 layout Word-parity | 🔶 em andamento (2026-07-07) | **F4.3 parcial (2026-07-07)**: `w:spacing` efetivo (cascata) aplicado no layout — altura de linha pela FONTE (auto = múltiplo de single via tabela de fatores TTF interina em `Draw._singleLineFactorByFont`; atLeast/exact em px), `before`/`after` no `offsetY` da 1ª linha do parágrafo (com guarda p/ `w:br`), ZERO do parágrafo herda fonte/tamanho efetivos (senão o defaultFont da shell dominava a linha). Campos novos em `IElement`: `lineSpacingRule/Value`, `paraSpacingBefore/After` (clone+copyAttr). ETP: 30→**24 págs** (Word: 19); resto do gap = tabelas (F4.5), TTF real (F4.10), indents de numeração (F4.2). **Nota (2026-07-07): usuário autorizou reestruturação agressiva com breaking changes — prioridade máxima em performance e fidelidade para documentos de 200-400 páginas.** **F4.5/F4.7 parcial (2026-07-07, 2º incremento)**: table paging portado do Draw.ts original E estendido para **rowspan atravessado** (trunca o rowspan na parte 1 e insere célula-continuação vazia na parte 2, como `vMerge continue`) — a tabela de 1.337 linhas/97.304px do TR agora divide em ~150 partes ≤ altura da página, com repeat header (`w:tblHeader`→`pagingRepeat`); altura de TD passou a incluir `offsetY` (before/after dos parágrafos da célula); `trHeight` sem hRule = atLeast (default OOXML), piso 20px p/ linhas sem trHeight. **F4.2 parcial**: `w:ind` efetivo (pPr direto ∨ nível de numeração via `numbering.levelOf`) → `paraIndentLeft/FirstLine` no `offsetX` das rows (1ª linha = left+firstLine/hanging; continuações = left). **Fidelidade medida**: TR 67→**184 págs** (Word: 140; overshoot ~30% = larguras de glifo do canvas, resolve com ce_fonts F4.10); ETP estável em 24 (Word: 19). E2E 39/39 + VM 20/20 verdes. |
| F4 (histórico anterior) | — | **F4.7a** campos `PAGE`/`NUMPAGES` dinâmicos: conversor extrai o formato do rodapé (`Página {pageNo} \| {pageCount}` + alinhamento/fonte do parágrafo do campo) e o app configura o `IPageNumber` do editor (sem duplicação com o texto em cache); **F4.5 parcial** bordas efetivas: `w:tblPr` parseado nos estilos de tabela (`ce_docx`), `FormatResolver.resolveTableBorders` (direto ∨ cadeia do `tblStyle`), conversor decide `TableBorder.all` + cor (ETP: `#BFBFBF` do Grid Table Light) ou `TableBorder.empty` + `borderTypes` por célula; **F4.6 parcial** distâncias de header/footer do `pgMar` aplicadas nas options. Pendentes: numeração real no layout (F4.2), spacing exato/widow-orphan (F4.3), tab stops (F4.4), tcBorders no nível de lado/cor por célula, table paging do POC, carimbo (F4.8), TTF (F4.10) |
...
| F5 desempenho | 🔶 parcial (2026-07-08) | **Table paging em passo único**: o split reescrito particiona a tabela inteira de uma vez (setup O(linhas) 1×) em vez de cortar uma parte por iteração re-executando o setup (era O(partes×linhas)). Reconstituição limpa as células-continuação sintéticas e restaura rowspans. **TR abre 63s→20s** com rendering idêntico (184 págs); ainda acima do orçamento G4 (<3s) — o resto é o layout O(documento) + `computePositionList` sobre 184 págs (resolve com F5.4 virtualização/posições incrementais). Guard de reuso de layout de célula por render. |
...
Progresso desta rodada, commitado e com push em origin/main:

O que entrou agora
F4.7a — Campos PAGE/NUMPAGES dinâmicos. O conversor detecta os campos no rodapé e extrai o formato real do documento (Página {pageNo} | {pageCount}), junto com alinhamento, fonte, tamanho e cor do parágrafo do campo. O app configura o IPageNumber do editor com isso — cada página agora mostra "Página X | Y" de verdade, atualizado pela paginação, em vez do resultado em cache ("Página 2 | 15" congelado em todas as páginas). O parágrafo do campo sai do rodapé estático para não duplicar, e quando um DOCX não tem campos o numerador da demo é desligado. Detalhe de fidelidade: no ETP o campo é alinhado à esquerda (o jc right estava noutro parágrafo) e no TR é centralizado — o teste cobre os dois.

F4.5 parcial — Bordas efetivas. O ce_docx agora parseia w:tblPr dentro dos estilos de tabela e o FormatResolver.resolveTableBorders resolve a cascata (tblBorders direto → cadeia basedOn do tblStyle). Com isso as 3 tabelas do ETP ganham as bordas cinza-claro #BFBFBF do estilo "Grid Table Light" (antes: cinza default), e tabelas sem bordas de tabela usam TableBorder.empty com as bordas por célula (borderTypes) que o render já suporta.

F4.6 parcial — Distâncias de header/footer do pgMar (426/454 twips no ETP, 567/230 no TR) aplicadas nas options do editor no abrir.

Verificação
20 testes VM (test/word/) + 29 no ce_docx + 2 E2E — o E2E confirma que o save continua byte-idêntico com as novas options (rodapé não é tocado no round-trip).
CI completo verde; roteiro (seção 0) e memória atualizados.
Estado do roteiro
F0 ✅ F1 ✅ F2 ✅ F3 ✅ F4 🔶 — os próximos itens da F4 são os profundos no motor de layout, cada um mexendo no core do Draw: F4.2 numeração multinível real no layout (substituir o marcador textual inline), F4.3 spacing exato/widow-orphan/keepNext na paginação, F4.4 tab stops, e o port do table paging do POC. Sugiro atacar o F4.2 na próxima rodada — os contadores (NumberingCounters) já estão prontos no ce_docx, falta integrá-los ao pipeline de linhas do editor.