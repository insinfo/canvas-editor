...
class EditorApp {
          ...
          DialogOptions(
            ...
            data: [
              ...
              DialogData(
                ...
                required: true,
                options: [
                  DialogOptionItem(label: 'Não repetir', value: '0'),
                  DialogOptionItem(label: 'Repetir', value: '1'),
                ],
              ),
              DialogData(
                type: 'number',
            ...
            onConfirm: (payload) {
              ...
              if (data == null ||
                  ...
                  sizeValue.isEmpty ||
                  opacityValue == null ||
                  opacityValue.isEmpty) {
                return;
              }
              final repeatValue = _findPayloadValue(payload, 'repeat');
              final horizontalGapValue =
                  _findPayloadValue(payload, 'horizontalGap');
              final verticalGapValue =
                  _findPayloadValue(payload, 'verticalGap');
              final watermark = IWatermark(
                ...
                gap: (repeatValue == '1' &&
                        horizontalGapValue != null &&
                        horizontalGapValue.isNotEmpty &&
                        verticalGapValue != null &&
                        verticalGapValue.isNotEmpty)
                    ? <double>[
          ...
          Dialog(
            DialogOptions(
              ...
              data: [
                ...
                DialogData(
                  type: 'textarea',
                  label: 'Conjunto de valores',
                  name: 'valueSets',
                  required: true,
                  height: 100,
                  placeholder:
                      'Digite o JSON do conjunto de valores. Ex.:\n[{"value":"Sim","code":"98175"}]',
                ),
              ],
              onConfirm: (payload) {
                final placeholder = _findPayloadValue(payload, 'placeholder');
            ...
            DialogOptions(
              ...
              data: [
                DialogData(
                  ...
                  name: 'code',
                  placeholder:
                      'Digite o valor padrão; separe múltiplos valores por vírgula',
                ),
                DialogData(
                  type: 'textarea',
                  label: 'Conjunto de valores',
          ...
          Dialog(
            DialogOptions(
              ...
              onConfirm: (payload) {
                ...
                final value = _findPayloadValue(payload, 'value') ?? '';
                final dateFormat =
                    _findPayloadValue(payload, 'dateFormat') ?? '';
                command.executeInsertControl(
                  IElement(
                    type: ElementType.control,
                    value: '',
        ...
        DialogOptions(
          ...
          data: [
            DialogData(
              ...
              height: 100,
              name: 'value',
              placeholder: 'Digite o texto LaTeX',
            ),
          ],
          onConfirm: (payload) {
            final value = _findPayloadValue(payload, 'value');
  ...
  void _setupDateControl() {
    ...
    dateDom.onClick.listen((_) {
      ...
      if (bodyRect != null &&
          optionRect.left + optionRect.width > bodyRect.width) {
        ...
        DialogOptions(
          ...
          data: [
            DialogData(
              ...
              required: true,
              options: [
                DialogOptionItem(label: 'URL', value: 'iframe'),
                DialogOptionItem(label: 'Vídeo', value: 'video'),
              ],
            ),
            DialogData(
              type: 'number',
              ...
              name: 'width',
              placeholder:
                  'Digite a largura (padrão: largura interna da página)',
            ),
            DialogData(
              type: 'number',
              label: 'Altura',
            ...
            DialogData(
              ...
              height: 100,
              name: 'srcdoc',
              placeholder: 'Digite o código HTML (somente para o tipo URL)',
            ),
          ],
          onConfirm: (payload) {
            ...
            if (blockType == BlockType.iframe) {
              ...
              block.iframeBlock =
                  IIFrameBlock(src: srcValue, srcdoc: srcdocValue);
  ...
  void _setupSearchAndReplace() {
    ...
    searchDom.onClick.listen((_) {
      ...
      if (bodyRect != null &&
          searchRect.left + collapseRect.width > bodyRect.width) {
  ...
  void _setupModeControl() {
    ...
    final modes = <Map<String, Object>>[
      ...
      {'mode': EditorMode.readonly, 'name': 'Modo somente leitura'},
      {'mode': EditorMode.form, 'name': 'Modo formulário'},
      {'mode': EditorMode.print, 'name': 'Modo de impressão'},
      {'mode': EditorMode.design, 'name': 'Modo de design'},
    ];
    var index = 0;
        ...
        DialogOptions(
          ...
          data: [
            DialogData(
              ...
              required: true,
              value: optionText,
              placeholder: 'Digite a configuração do editor',
            ),
          ],
          onConfirm: (payload) {
  ...
  Map<String, dynamic> _editorOptionSnapshot(IEditorOption options) {
    return <String, dynamic>{
      ...
      if (options.highlightAlpha != null)
        'highlightAlpha': options.highlightAlpha,
      if (options.highlightMarginHeight != null)
        'highlightMarginHeight': options.highlightMarginHeight,
      if (options.margins != null)
        'margins': List<double>.from(options.margins!),
      if (options.maskMargin != null)
        'maskMargin': List<double>.from(options.maskMargin!),
      if (options.renderMode != null) 'renderMode': options.renderMode!.name,
      if (options.wordBreak != null) 'wordBreak': options.wordBreak!.name,
      if (options.pageOuterSelectionDisable != null)
        'pageOuterSelectionDisable': options.pageOuterSelectionDisable,
      if (options.defaultHyperlinkColor != null)
        'defaultHyperlinkColor': options.defaultHyperlinkColor,
      if (options.letterClass != null)
        'letterClass': List<String>.from(options.letterClass!),
      if (options.contextMenuDisableKeys != null)
        'contextMenuDisableKeys':
            List<String>.from(options.contextMenuDisableKeys!),
      if (options.shortcutDisableKeys != null)
        'shortcutDisableKeys': List<String>.from(options.shortcutDisableKeys!),
    };
  }
  ...
  IUpdateOption? _parseUpdateOption(dynamic value) {
    ...
    hasValue |=
        assignString('defaultColor', (raw) => option.defaultColor = raw);
    hasValue |=
        assignNum('defaultSize', (raw) => option.defaultSize = raw.toInt());
    hasValue |= assignNum('minSize', (raw) => option.minSize = raw.toInt());
    hasValue |= assignNum('maxSize', (raw) => option.maxSize = raw.toInt());
    hasValue |= assignNum(
        'defaultRowMargin', (raw) => option.defaultRowMargin = raw.toDouble());
    hasValue |= assignNum('defaultBasicRowMarginHeight',
        (raw) => option.defaultBasicRowMarginHeight = raw.toDouble());
    hasValue |= assignNum(
        'defaultTabWidth', (raw) => option.defaultTabWidth = raw.toDouble());
    hasValue |=
        assignString('underlineColor', (raw) => option.underlineColor = raw);
    hasValue |=
        assignString('strikeoutColor', (raw) => option.strikeoutColor = raw);
    hasValue |= assignString('rangeColor', (raw) => option.rangeColor = raw);
    hasValue |=
        assignNum('rangeAlpha', (raw) => option.rangeAlpha = raw.toDouble());
    hasValue |= assignNum(
        'rangeMinWidth', (raw) => option.rangeMinWidth = raw.toDouble());
    hasValue |= assignNum(
        'highlightAlpha', (raw) => option.highlightAlpha = raw.toDouble());
    hasValue |= assignNum('highlightMarginHeight',
        (raw) => option.highlightMarginHeight = raw.toDouble());
    ...
    if (value['pageOuterSelectionDisable'] is bool) {
      option.pageOuterSelectionDisable =
          value['pageOuterSelectionDisable'] as bool;
      hasValue = true;
    }

    hasValue |= assignString(
        'defaultHyperlinkColor', (raw) => option.defaultHyperlinkColor = raw);
    ...
    if (value['contextMenuDisableKeys'] is List) {
      option.contextMenuDisableKeys =
          value['contextMenuDisableKeys'].cast<String>();
      hasValue = true;
    }
  ...
  T _requireElementFrom<T extends Element>(Element parent, String selector) {
    ...
    throw StateError(
        'Could not find element for selector $selector within ${parent.runtimeType}');
  }
  ...
  void _setupSeparatorAndPageBreakControls() {
    ...
    separatorOptionDom.onMouseDown.listen((event) {
      ...
      final dashList = dashValue != null && dashValue.isNotEmpty
          ...
          : <int>[];
      final isSolid =
          dashList.isEmpty || dashList.every((element) => element == 0);
      final payload = isSolid
          ? <num>[]
          : dashList.map((value) => value.toDouble()).toList();
  ...
  void _setupTableControls() {
    final tableDom = document.querySelector('.menu-item__table');
    final tablePanelContainer =
    ...
    if (existingTableTitle == null) {
      ...
      if (titleContainer != null) {
        titleContainer.nodes.insert(0, tableTitle);
      }
    }
