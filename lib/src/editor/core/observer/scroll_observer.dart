...
class ScrollObserver {
  ...
  IPageVisibleInfo? _getPagingVisibleInfo() {
    ...
    if (identical(_scrollContainer, document)) {
      ...
      viewportBottom =
          containerRect.bottom.toDouble() - pageContainerRect.top.toDouble();
    } else {
      return null;
    }
  ...
  void _handleScroll() {
    ...
    final int targetPage = info.visiblePageNoList.isEmpty
        ? info.intersectionPageNo
        : info.visiblePageNoList.reduce(math.max);
    draw.ensureProgressiveLayoutForPage(targetPage);
  }
}
