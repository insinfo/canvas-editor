...
class Zone {
  ...
  void setZone(EditorZone zone) {
    ...
    // F4.6: a lista editável do header/footer passa a ser a VARIANTE da
    // página corrente (first/even) — edição in-place estilo Word.
    final dynamic headerFrame = _draw.getHeader();
    final dynamic footerFrame = _draw.getFooter();
    final int currentPage = (_draw.getPageNo() as num?)?.toInt() ?? 0;
    try {
      if (zone == EditorZone.header) {
        headerFrame?.setActiveVariantForPage(currentPage);
      } else {
        headerFrame?.resetActiveVariant();
      }
      if (zone == EditorZone.footer) {
        footerFrame?.setActiveVariantForPage(currentPage);
      } else {
        footerFrame?.resetActiveVariant();
      }
    } catch (_) {}
