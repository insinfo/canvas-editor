...
class TableOperate {
    ...
    if (startIndex >= 0 &&
      ...
      try {
        final List<dynamic> positionList =
            _position?.getPositionList() as List<dynamic>? ?? <dynamic>[];
  ...
  void insertTableBottomRow() {
    ...
    final ITr currentTr = trList[trIndex];
    final ITr anchorTr =
        trIndex == trList.length - 1 ? currentTr : trList[trIndex + 1];
  ...
  void adjustColWidth(IElement element) {
    ...
    final double adjustWidth = (colgroupWidth - width) / adjustableCols.length;
  ...
  void deleteTableRow() {
    ...
    final int curTdRowIndex = currentTr.tdList[tdIndex].rowIndex ?? trIndex;
  ...
  void deleteTableCol() {
    ...
    final bool hasMultiCol = trList.any((ITr tr) => tr.tdList.length > 1);
    if (!hasMultiCol) {
      deleteTable();
      return;
    }
  ...
  void tableBorderColor(String payload) {
    ...
    final IElement element = elementList[index];
    final String defaultColor = _options.table?.defaultBorderColor ?? payload;
