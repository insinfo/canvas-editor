...
class Search {
  ...
  int? searchNavigateNext() {
    ...
    if (searchNavigateIndex != null &&
        searchNavigateIndex! + keywordLength > searchMatchList.length) {
      searchNavigateIndex = searchMatchList.length - keywordLength;
    }
    return searchNavigateIndex;
  }
  ...
  List<ISearchResult> getMatchList(
      String payload, List<IElement> originalElementList) {
    ...
    while (elementIndex < length) {
      final int? tableIndex = tablePointer < tableIndexList.length
          ? tableIndexList[tablePointer]
          : null;
    ...
    void searchClosure(
        String? value, EditorContext type, List<IElement> elementList,
      ...
      final String text =
          mapped.where((String entry) => entry.isNotEmpty).join().toLowerCase();
      ...
      for (final int startIndex in startIndexList) {
        ...
        for (int i = 0; i < value.length; i++) {
          final int elementIndex = startIndex + i + (restArgs?.startIndex ?? 0);
          matches.add(
            ISearchResult(
              type: type,
              index: elementIndex,
  ...
  void render(CanvasRenderingContext2D ctx, int pageIndex) {
    ...
    final String navigateColor = _options.searchNavigateMatchColor ??
        _options.searchMatchColor ??
        '#ffb74d';
    final List<IElementPosition> positionList =
        _position.getOriginalPositionList();
    final List<IElement> elementList = _draw.getOriginalElementList();
    ...
    for (int i = 0; i < searchMatchList.length; i++) {
      ...
      if (navigateIndexSet.contains(i)) {
        ...
        if (previousMatch == null ||
            previousMatch.groupId != searchMatch.groupId) {
  ...
  void replace(String? payload, [IReplaceOption? option]) {
    ...
    List<ISearchResult> matchList =
        List<ISearchResult>.from(getSearchMatchList());
    ...
    if (replaceIndex != null && isNumber(replaceIndex)) {
      ...
      final List<List<ISearchResult>> groupedMatchList =
          <List<ISearchResult>>[];
      ...
      if (match.type == EditorContext.table) {
        ...
        for (int p = 0; p < payload.length; p++) {
          ...
          if (p == 0) {
            ...
            final IElement cloned = element_utils
                .cloneElementList(<IElement>[tableElementItem]).first;
            cloned.value = value;
