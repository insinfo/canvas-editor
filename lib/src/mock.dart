import 'editor/dataset/enum/common.dart';
import 'editor/dataset/enum/control.dart';
import 'editor/dataset/enum/element.dart';
import 'editor/dataset/enum/list.dart';
import 'editor/dataset/enum/title.dart';
import 'editor/interface/editor.dart';
import 'editor/interface/element.dart';
import 'editor/interface/watermark.dart';
import 'editor/interface/page_number.dart';
import 'editor/interface/placeholder.dart';
import 'editor/interface/zone.dart';
import 'editor/interface/control.dart';
import 'editor/interface/table/td.dart';
import 'editor/interface/table/tr.dart';
import 'editor/interface/table/colgroup.dart';

String _medicalReportText() =>
		'主诉：\n发热三天，咳嗽五天。\n现病史：\n患者于三天前无明显诱因，感冒后发现面部水肿，无皮疹，尿量减少，出现乏力，在外治疗无好转，现来我院就诊。\n既往史：\n有糖尿病10年，有高血压2年，有传染性疾病1年。报告其他既往疾病。\n流行病史：\n否认14天内接触过确诊患者、疑似患者、无症状感染者及其密切接触者；否认14天内去过以下场所：水产、肉类批发市场，农贸市场，集市，大型超市，夜市；否认14天内与以下场所工作人员密切接触：水产、肉类批发市场，农贸市场，集市，大型超市；否认14天内周围（如家庭、办公室）有2例以上聚集性发病；否认14天内接触过有发热或呼吸道症状的人员；否认14天内自身有发热或呼吸道症状；否认14天内接触过纳入隔离观察的人员及其他可能与新冠肺炎关联的情形；陪同家属无以上情况。\n体格检查：\nT：39.5℃，P：80bpm，R：20次/分，BP：120/80mmHg；\n辅助检查：\n2020年6月10日，普放：血细胞比容36.50%（偏低）40～50；单核细胞绝对值0.75*10/L（偏高）参考值：0.1～0.6；\n门诊诊断：处置治疗：电子签名：【】\n其他记录：';

Map<int, String> _buildTitleMap(String text, List<String> titles) {
	final map = <int, String>{};
	for (final title in titles) {
		final index = text.indexOf(title);
		if (index >= 0) {
			map[index] = title;
		}
	}
	return map;
}

Set<int> _spanIndices(String text, List<String> patterns) {
	final indices = <int>{};
	for (final pattern in patterns) {
		final start = text.indexOf(pattern);
		if (start >= 0) {
			for (var offset = 0; offset < pattern.length; offset++) {
				indices.add(start + offset);
			}
		}
	}
	return indices;
}

List<IElement> _buildBaseElements(String text) {
	final titles = [
		'主诉：',
		'现病史：',
		'既往史：',
		'流行病史：',
		'体格检查：',
		'辅助检查：',
		'门诊诊断：',
		'处置治疗：',
		'电子签名：',
		'其他记录：',
	];

	final titleMap = _buildTitleMap(text, titles);
	final colorIndex = _spanIndices(text, ['传染性疾病']);
	final highlightIndex = _spanIndices(text, ['血细胞比容']);

	final elements = <IElement>[];
	var index = 0;
	while (index < text.length) {
		final value = text[index];
		final title = titleMap[index];

		if (title != null) {
			elements.add(
				IElement(
					value: '',
					type: ElementType.title,
					level: TitleLevel.first,
					valueList: [
						IElement(
							value: title,
							size: 18,
						),
					],
				),
			);
			index += title.length;
			continue;
		}

		if (colorIndex.contains(index)) {
			elements.add(
				IElement(
					value: value,
					color: '#FF0000',
					size: 16,
				),
			);
		} else if (highlightIndex.contains(index)) {
			elements.add(
				IElement(
					value: value,
					highlight: '#F2F27F',
					groupIds: ['1'],
					size: 16,
				),
			);
		} else {
			elements.add(
				IElement(
					value: value,
					size: 16,
				),
			);
		}
		index++;
	}

	return elements;
}

List<IElement> _mockElementList() {
	final elementList = _buildBaseElements(_medicalReportText());

	elementList.insert(
		12,
		IElement(
			value: '',
			type: ElementType.control,
			control: IControl(
				conceptId: '1',
				type: ControlType.text,
				value: null,
				placeholder: '其他补充',
				prefix: '{',
				postfix: '}',
			valueSets: [],
				flexDirection: FlexDirection.row,
			),
		),
	);

		elementList.insert(
			94,
			IElement(
			value: '',
			type: ElementType.control,
			control: IControl(
				conceptId: '2',
				type: ControlType.select,
				value: null,
				code: null,
				placeholder: '有无',
				prefix: '{',
				postfix: '}',
					valueSets: [
					IValueSet(value: '有', code: '98175'),
					IValueSet(value: '无', code: '98176'),
					IValueSet(value: '不详', code: '98177'),
				],
				flexDirection: FlexDirection.row,
			),
		),
	);

	elementList.insert(
		116,
		IElement(
			value: '',
			type: ElementType.hyperlink,
					valueList: [
				IElement(value: '新', size: 16),
				IElement(value: '冠', size: 16),
				IElement(value: '肺', size: 16),
				IElement(value: '炎', size: 16),
			],
			url: 'https://hufe.club/canvas-editor',
		),
	);

	elementList.insert(
		335,
		IElement(
			value: '',
			type: ElementType.control,
			control: IControl(
				conceptId: '6',
				type: ControlType.text,
				value: null,
				placeholder: '内容',
				preText: '其他：',
				postText: '。',
					valueSets: [],
				flexDirection: FlexDirection.row,
			),
		),
	);

	elementList.insert(
		346,
		IElement(
			value: '∆',
			color: '#FF0000',
			type: ElementType.subscript,
		),
	);

	elementList.insert(
		430,
		IElement(
			value: '9',
			type: ElementType.superscript,
		),
	);

	elementList.insert(
		451,
		IElement(
					value: '',
					type: ElementType.list,
					listType: ListType.ordered,
					valueList: [
				IElement(
					value: '高血压\n糖尿病\n病毒性感冒\n过敏性鼻炎\n过敏性鼻息肉',
				),
			],
		),
	);

	elementList.insert(
		453,
		IElement(
					value: '',
					type: ElementType.list,
					listType: ListType.ordered,
					valueList: [
				IElement(
					value:
							'超声引导下甲状腺细针穿刺术；\n乙型肝炎表面抗体测定；\n膜式病变细胞采集术、后颈皮下肤层；',
				),
			],
		),
	);

	elementList.insert(
		456,
		IElement(
			value:
					'data:image/png;base64,',
			width: 89,
			height: 32,
			id: 'signature',
			type: ElementType.image,
		),
	);

	elementList.add(
		IElement(
			value: '',
			type: ElementType.table,
					colgroup: [
				IColgroup(width: 180),
				IColgroup(width: 80),
				IColgroup(width: 130),
				IColgroup(width: 130),
			],
			trList: [
						ITr(
					height: 40,
					tdList: [
						ITd(
							colspan: 1,
							rowspan: 2,
									value: [
								IElement(value: '1', size: 16),
								IElement(value: '.', size: 16),
							],
						),
						ITd(
							colspan: 1,
							rowspan: 1,
									value: [
								IElement(value: '2', size: 16),
								IElement(value: '.', size: 16),
							],
						),
						ITd(
							colspan: 2,
							rowspan: 1,
									value: [
								IElement(value: '3', size: 16),
								IElement(value: '.', size: 16),
							],
						),
					],
				),
				ITr(
					height: 40,
					tdList: [
						ITd(
							colspan: 1,
							rowspan: 1,
									value: [
								IElement(value: '4', size: 16),
								IElement(value: '.', size: 16),
							],
						),
						ITd(
							colspan: 1,
							rowspan: 1,
									value: [
								IElement(value: '5', size: 16),
								IElement(value: '.', size: 16),
							],
						),
						ITd(
							colspan: 1,
							rowspan: 1,
									value: [
								IElement(value: '6', size: 16),
								IElement(value: '.', size: 16),
							],
						),
					],
				),
				ITr(
					height: 40,
					tdList: [
						ITd(
							colspan: 1,
							rowspan: 1,
										value: [
								IElement(value: '7', size: 16),
								IElement(value: '.', size: 16),
							],
						),
						ITd(
							colspan: 1,
							rowspan: 1,
										value: [
								IElement(value: '8', size: 16),
								IElement(value: '.', size: 16),
							],
						),
						ITd(
							colspan: 1,
							rowspan: 1,
										value: [
								IElement(value: '9', size: 16),
								IElement(value: '.', size: 16),
							],
						),
						ITd(
							colspan: 1,
							rowspan: 1,
										value: [
								IElement(value: '1', size: 16),
								IElement(value: '0', size: 16),
								IElement(value: '.', size: 16),
							],
						),
					],
				),
			],
		),
	);

				elementList.addAll([
					IElement(value: '是否同意以上内容：'),
					IElement(
			value: '',
			type: ElementType.control,
			control: IControl(
				conceptId: '3',
				type: ControlType.checkbox,
				code: '98175',
							value: null,
				valueSets: [
					IValueSet(value: '同意', code: '98175'),
					IValueSet(value: '否定', code: '98176'),
				],
				flexDirection: FlexDirection.row,
			),
		),
		IElement(value: '\n'),
	]);

		elementList.addAll([
			IElement(value: '医学公式：'),
			IElement(value: '{E_k} = hv - {W_0}', type: ElementType.latex),
			IElement(value: '\n'),
	]);

		elementList.addAll([
			IElement(value: '签署日期：'),
			IElement(
			value: '',
			type: ElementType.control,
			control: IControl(
				conceptId: '5',
				type: ControlType.date,
				value: [
					IElement(value: '2022-08-10 17:30:01'),
				],
				placeholder: '签署日期',
				valueSets: [],
				flexDirection: FlexDirection.row,
			),
		),
		IElement(value: '\n'),
	]);

		elementList.addAll([
			IElement(value: '患者签名：'),
			IElement(
			value: '',
			type: ElementType.control,
			control: IControl(
				conceptId: '4',
				type: ControlType.text,
				value: null,
				placeholder: '',
				prefix: '\u200c',
				postfix: '\u200c',
				minWidth: 160,
				underline: true,
						valueSets: [],
				flexDirection: FlexDirection.row,
			),
		),
	]);

			elementList.addAll([
		IElement(value: '\n'),
		IElement(value: '', type: ElementType.tab),
		IElement(value: 'E', size: 16),
		IElement(value: 'O', size: 16),
		IElement(value: 'F', size: 16),
	]);

	return elementList;
}

final List<IElement> data = List<IElement>.unmodifiable(_mockElementList());

class EditorComment {
	final String id;
	final String content;
	final String userName;
	final String rangeText;
	final String createdDate;

	const EditorComment({
		required this.id,
		required this.content,
		required this.userName,
		required this.rangeText,
		required this.createdDate,
	});
}

const List<EditorComment> commentList = [
	EditorComment(
		id: '1',
		content: '红细胞比容（HCT）是指每单位容积中红细胞所占全血容积的比值，用于反映红细胞和血浆的比例。',
		userName: 'Hufe',
		rangeText: '血细胞比容',
		createdDate: '2023-08-20 23:10:55',
	),
];

final IEditorOption options = IEditorOption(
	margins: [100, 120, 100, 120],
	watermark: IWatermark(data: 'CANVAS-EDITOR', size: 120),
	pageNumber: IPageNumber(format: '第{pageNo}页/共{pageCount}页'),
	placeholder: IPlaceholder(data: '请输入正文'),
	zone: IZoneOption(tipDisabled: false),
	maskMargin: [60, 0, 30, 0],
);