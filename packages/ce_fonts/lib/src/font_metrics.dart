...
FontMetrics parseTtfMetrics(
  ...
  for (int i = 0; i < numSub; i++) {
    ...
    final bool unicodeBmp = (platform == 3 && (encoding == 1 || encoding == 0)) ||
...
class _CmapFormat4 {
  ...
  final int idRangeOffsetPos; // offset absoluto do início do array idRangeOffset
  ...
  int glyphOf(int cp) {
    for (int i = 0; i < segCount; i++) {
      ...
      final int glyphIndexAddr = idRangeOffsetPos +
          i * 2 +
          rangeOffset +
          (cp - startCodes[i]) * 2;
