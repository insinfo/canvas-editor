...
class WpSpacing {
  ...
  const WpSpacing({this.beforeTwips, this.afterTwips, this.line, this.lineRule});
...
class WpIndent {
  ...
  const WpIndent(
      {this.leftTwips, this.rightTwips, this.firstLineTwips, this.hangingTwips});
...
class WpTableWidth {
  ...
  static WpTableWidth? fromXml(XmlElement? el) {
    if (el == null) return null;
    return WpTableWidth(value: _intVal(el, 'w:w'), type: el.getAttribute('w:type'));
  }
}
...
class WpTableProperties {
  ...
  const WpTableProperties(
      {this.styleId, this.width, this.jc, this.borders, this.indentTwips, this.layout});
...
class WpTableRowProperties {
  ...
  const WpTableRowProperties(
      {this.heightTwips, this.heightRule, this.tblHeader = false, this.cantSplit = false});
...
class WpTableCellProperties {
  ...
  const WpTableCellProperties(
      {this.width, this.gridSpan, this.vMerge, this.borders, this.shading, this.vAlign});
...
class WpSettings {
  ...
  static WpSettings fromXml(XmlElement? root) {
    ...
    return WpSettings(
      ...
      evenAndOddHeaders: _onOff(root.firstChild('w:evenAndOddHeaders')) ?? false,
