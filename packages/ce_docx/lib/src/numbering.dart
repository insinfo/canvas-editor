...
class WpNumberingLevel {
  ...
  static WpNumberingLevel fromXml(XmlElement el) {
    return WpNumberingLevel(
      ...
      start: int.tryParse(
              el.firstChild('w:start')?.getAttribute('w:val') ?? '') ??
          1,
      numFmt:
          el.firstChild('w:numFmt')?.getAttribute('w:val') ?? 'decimal',
      lvlText: el.firstChild('w:lvlText')?.getAttribute('w:val') ?? '',
      lvlJc: el.firstChild('w:lvlJc')?.getAttribute('w:val'),
      indent: WpIndent.fromXml(
          el.firstChild('w:pPr')?.firstChild('w:ind')),
...
class NumberingCounters {
  ...
  String? next(int numId, int ilvl) {
    ...
    for (var i = 1; i <= 9; i++) {
      ...
      final fmt =
          numbering.levelOf(numId, lvlIndex)?.numFmt ?? 'decimal';
